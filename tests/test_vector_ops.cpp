// vkm — Unity-style vector ops: angle / project / projectOnPlane /
// moveTowards / rotateTowards (plus a reflect sanity check).
...
int main() {
    ...
    CHECK(near(projectOnPlane(float3{1, 2, 3}, float3{0, 0, 1}), float3{1, 2, 0})); // drop Z
    CHECK(near(projectOnPlane(float3{1, 2, 3}, float3{0, 0, 0}), float3{1, 2, 3})); // zero -> v
    // identity: projection + in-plane component reconstruct the original vector
    {
        float3 v{1, 2, 3}, n{0, 1, 0};
        CHECK(near(project(v, n) + projectOnPlane(v, n), v));
    }
    ...
    // ---- moveTowards (scalar + vector) ----
    CHECK(near(moveTowards(0.0f, 10.0f, 3.0f), 3.0f));   // step
    CHECK(near(moveTowards(0.0f, 10.0f, 100.0f), 10.0f)); // clamp, no overshoot
    CHECK(near(moveTowards(10.0f, 0.0f, 3.0f), 7.0f));   // toward smaller
    CHECK(near(moveTowards(float3{0, 0, 0}, float3{10, 0, 0}, 3.0f), float3{3, 0, 0}));
    ...
    CHECK(near(rotateTowards(float3{1, 0, 0}, float3{0, 5, 0}, pi, 2.0f), float3{0, 3, 0}));
    // antiparallel with full budget -> lands on target (the degenerate-axis path)
    CHECK(near(rotateTowards(float3{1, 0, 0}, float3{-1, 0, 0}, pi, 0.0f), float3{-1, 0, 0}));
    // antiparallel, partial turn -> stays unit length and is perpendicular to the start
    {
        float3 r = rotateTowards(float3{1, 0, 0}, float3{-1, 0, 0}, pi / 2, 0.0f);
    ...
    CHECK(near(rotateTowards(float3{0, 0, 0}, float3{0, 3, 0}, pi, 2.0f), float3{0, 2, 0}));
